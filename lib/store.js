...
function monthlyFactor(cadence) {
  ...
  if (c.includes('year') || c === 'annual' || c === 'annually') return 1 / 12;
  if (c === 'semi-annual' || c === 'semi_annual' || c.includes('semi')) return 1 / 6;
  if (c.includes('quarter')) return 1 / 3;
  // biweekly must be checked before the generic 'week' match
  if (c.includes('biweek') || c.includes('bi-week') || c.includes('fortnight')) return 2.1725;
  if (c.includes('week')) return 4.345;
  return 1;
}
...
function mapLocalToDb(userId, sub) {
  ...
  return {
    ...
    // Synced so the backend can fire a server-side "trial ending" push
    // (complements the on-device reminder, works when the app is closed).
...
function mapDbToLocal(row) {
  ...
  return {
    ...
    status:       row.is_active === false ? 'cancelled' : (row.user_status ?? null),
...
export const useStore = create((set, get) => ({
  ...
  notificationSettings: {
    ...
    daysBefore: [14, 7, 3, 1],
    timeOfDay: '09:00',
    newSubAlertsEnabled: true,
  },

  // Last result of scheduleAll() so silent scheduling failures (denied perms,
  // per-item throws) are observable instead of being swallowed. See dev-tools.
  notifyDiag: null,
  ...
  // --- Profile ---
  loadProfile: async () => {
    // Priority: profiles table (Supabase, cross-device) > user_metadata > AsyncStorage (local cache) > random
    const user = get().user;
    const meta = user?.user_metadata || {};

    const stored = await safeGetJSON(PROFILE_KEY, null);

    // Try to fetch avatar_url from the profiles table first
    let profilesAvatar = null;
    if (supabase && user?.id) {
      try {
        const { data } = await supabase.from('profiles').select('avatar_url').eq('id', user.id).single();
        if (data?.avatar_url) profilesAvatar = data.avatar_url;
      } catch {
        // no-op
      }
    }

    // avatarUrl (https://) is a remote Supabase Storage URL - safe cross-device
    // avatarUri in user_metadata could be a stale local path from another install - discard those
    // avatarUri in AsyncStorage is always from this install - keep it even if it's a local file:// path
    const metaAvatar = meta?.avatarUrl || meta?.avatarUri || null;
    const isMetaLocalPath = metaAvatar && (
      metaAvatar.startsWith('file://') ||
      metaAvatar.startsWith('/var/') ||
      metaAvatar.startsWith('/data/')
    );
    const safeMetaAvatar = isMetaLocalPath ? null : metaAvatar;
    const storedAvatar = profilesAvatar || safeMetaAvatar || stored?.avatarUri || null;

    const next = {
      username: meta?.username || stored?.username || randomUsername(),
      name: meta?.name || stored?.name || null,
      avatarUri: storedAvatar,
    };
    const devPro = isDevProUsername(next.username);
    set({ profile: next, ...(devPro ? { pro: true } : {}) });
  ...
  updateProfile: async (patch) => {
    ...
    // Also write avatar_url to the profiles table so it persists across devices
    if (typeof patch?.avatarUri !== "undefined" && supabase) {
      const val = patch.avatarUri ?? null;
      if (!val || val.startsWith('http')) {
        const userId = get().user?.id;
        if (userId) {
          try {
            await supabase.from('profiles').upsert({ id: userId, avatar_url: val }, { onConflict: 'id' });
          } catch {
            // no-op (local-first)
          }
        }
  ...
  updateNotificationSettings: async (patch) => {
    ...
    // Mirror the new-subscription-alerts preference to push_tokens so the
    // backend honors it end-to-end (best-effort, non-blocking).
    if (patch && Object.prototype.hasOwnProperty.call(patch, 'newSubAlertsEnabled')) {
      import('./push')
        .then((m) => m.updateNewSubAlertPref?.(patch.newSubAlertsEnabled))
        .catch(() => {});
    }
  },
  ...
  updateBill: async (id, patch) => {
    ...
    // When cancelling a bill, record saving (best-effort)
    let savedEntry = null;
    if (patch.active === false && String(patch.status || "").toLowerCase().includes("cancel") && prev?.amount) {
  ...
  getRecapRecommendations: () => {
    ...
    const out = [];
    for (const s of get().subs.filter(s => s?.active !== false && !!s?.nextRenewal)) {
  ...
  getActionFeed: () => {
    ...
    // Surface the Optimize trick on Home once there are enough subscriptions
    // that there's likely something to save (duplicates, annual switches, etc.).
    const activeSubCount = (get().subs || []).filter(
      (s) => s.active !== false && !String(s.status || '').toLowerCase().includes('cancel')
    ).length;
    if (activeSubCount >= 3) {
      actions.push({
        kind: 'recommendation',
        title: 'Find ways to save',
        detail: 'Review duplicates, cheaper plans & annual switches.',
        href: '/optimize',
        priority: 2,
      });
    }
    ...
    for (const s of get().subs) {
      ...
      if (String(s.status || '').toLowerCase().includes('cancel')) continue;
  ...
  syncNow: async ({ source = 'manual' } = {}) => {
    ...
    try {
      const res = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .or('user_status.eq.confirmed,user_status.eq.cancelled');
    ...
    // Detect price changes vs current state and fire notifications (best-effort)
    const prevSubsMap = new Map((get().subs ?? []).map((s) => [s.id, s]));
    for (const s of hydrated) {
      const prev = prevSubsMap.get(s.id);
      if (prev && Number(prev.amount) > 0 && Number(s.amount) !== Number(prev.amount)) {
        schedulePriceChangeNotification({
          brand: s.merchant || s.title || 'Subscription',
          domain: s.domain || null,
          oldAmount: Number(prev.amount),
          newAmount: Number(s.amount),
    ...
    scheduleAll({
      ...
      .then((res) => {
        set({ notifyDiag: { at: nowISO(), ...res } });
        if (__DEV__ && (res?.error || res?.failed)) {
          console.warn('[notifications] schedule issues:', res?.error || '', `failed=${res?.failed ?? 0}`);
        }
      })
      .catch((e) => {
        set({ notifyDiag: { at: nowISO(), scheduled: 0, error: e?.message || String(e) } });
        if (__DEV__) console.warn('[notifications] scheduleAll threw:', e?.message || e);
      });
  ...
  addSub: async (sub) => {
    ...
    set({ subs: next });
    track("subscription_added", { merchant: local.merchant, cadence: local.cadence });
    await safeSetJSON(SUBS_KEY, next);
  ...
  updateSub: async (id, patch) => {
    ...
    // When cancelling, record saving (best-effort)
    let savedEntry = null;
    if (patch.active === false && String(patch.status || "").toLowerCase().includes("cancel") && prev?.amount) {
      savedEntry = await get().recordSaving({
        id: prev.id,
