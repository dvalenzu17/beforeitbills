...
// OAuth 2.0 "Web" client ID. REQUIRED for native Google Sign-In to return an
// idToken (the token Supabase's signInWithIdToken consumes). On Android, without
// webClientId, GoogleSignin returns no idToken and sign-in fails. The idToken's
// audience becomes this web client ID, so it must also be listed in Supabase's
// Google provider "Authorized Client IDs".
export const GOOGLE_WEB_CLIENT_ID =
  process.env.EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID ||
  process.env.EXPO_PUBLIC_GOOGLE_OAUTH_WEB_CLIENT_ID || // legacy fallback
  '';
