 ...
 */
async function newSubAlertsEnabled() {
  // Read the user's toggle from the main store. Dynamic import avoids any
  // module-load ordering issues. Defaults to on if unavailable.
  try {
    const { useStore } = await import("./store");
    return useStore.getState?.().notificationSettings?.newSubAlertsEnabled !== false;
  } catch {
    return true;
  }
}
...
async function notifyNewCandidates({ candidates, alertedKeys, seeded }) {
  ...
  // Only fire when the user has the toggle on. Either way we advance the known
  // set so re-enabling later never fires a backlog of old candidates.
  if (fresh.length && (await newSubAlertsEnabled())) {
    try {
