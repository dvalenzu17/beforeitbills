  ...
  scanAccount: async (id, opts = {}) => {
    ...
    try {
      ...
      if (isImap) {
        ...
        scanResult = await runImapScan({ provider, user, pass, daysBack: scanDaysBack });
      } else {
        scanResult = await runGmailScan({ daysBack: scanDaysBack, force: opts.force });
      }
  ...
  runScan: async (opts = {}) => {
    ...
    try {
      ...
      if (isImap) {
        ...
        scanResult = await runImapScan({ provider, user, pass, daysBack: fallbackDaysBack });
      } else {
        scanResult = await runGmailScan({ daysBack: fallbackDaysBack });
      }
