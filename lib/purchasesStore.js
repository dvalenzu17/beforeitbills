...
function getConfig() {
  ...
  return {
    ...
    entitlement: extra?.revenuecatEntitlementId ?? 'beforeitbills_pro',
  };
}
...
export const usePurchasesStore = create((set, get) => ({
  ...
  restore: async () => {
    ...
    try {
      ...
      // Always fetch fresh customer info after restore — restorePurchases()
      // may return a stale snapshot without active entitlements populated
      const fresh = await Purchases.getCustomerInfo();
      set({ customerInfo: fresh, isPro: isProActive(fresh), loading: false });
      return { ok: true };
    } catch (e) {
      const msg = e?.message ?? String(e);
      set({ loading: false, error: msg });
