...
export const usePurchasesStore = create((set, get) => ({
  ...
  init: async () => {
    try {
      ...
      // react-native-purchases@10 calls StoreKit APIs during configure() that
      // may throw an NSException on iOS 26 (new entitlement enforcement).
      // isConfigured guard prevents double-configure on hot reload.
      const iosMajor = Platform.OS === 'ios' ? parseInt(String(Platform.Version), 10) : 0;
      if (iosMajor >= 26) {
        // Skip RevenueCat on iOS 26 until SDK ships iOS 26 compatible release.
        set({ ready: true, isPro: false, customerInfo: null });
        return;
      }

      if (debug && Purchases?.setLogLevel) Purchases.setLogLevel(LOG_LEVEL.VERBOSE);

      if (!Purchases.isConfigured) Purchases.configure({ apiKey });
