...
export async function isOnboardingDone(user) {
  ...
  // If we have a user but their metadata doesn't have onboarding_done,
  // they are a new account — clear any stale cache from a previous user
  if (user) {
    AsyncStorage.removeItem(KEY).catch(() => {});
    return false;
  }

  // No user (guest/unauthenticated) — fall back to local cache
  try {
