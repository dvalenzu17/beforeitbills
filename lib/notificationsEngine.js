...
import { computeBillNextDue } from "./dateMath";
import { resolveReminderTarget } from "./reminderPlan";
...
function buildTrialCopy(name, windowDays, date, amount, currency) {
  const dateStr = fmtDateFriendly(date);
  const amtStr = fmtAmt(amount, currency);
  ...
  const title = `${name} - trial ends ${when}`;
  const parts = ["Cancel before you're charged"];
  if (amtStr) parts.push(amtStr);
  if (dateStr) parts.push(dateStr);
  const body = parts.join(" · ");
...
export async function buildUpcoming({
  ...
  for (const s of subs) {
    // Skip cancelled subs; pick trial-end vs renewal date with the same
    // fallback chain the UI uses (see reminderPlan.js).
    const target = resolveReminderTarget(s);
    if (!target) continue;
    const { kind, baseRaw } = target;

    const base = parseDateFlexible(baseRaw);
    if (!base) continue;

    const id = String(s.id ?? "");
    const name = s.merchant || s.title || "Subscription";
    ...
    // per-item pause
    const paused = await isPausedItem({ kind, id });
    if (paused) continue;
    ...
    for (const w of daysBefore) {
      ...
      const muted = await isMutedCycle({ kind, id, dateISO });
      if (muted) continue;

      list.push({ kind, id, name, amount, currency, domain, when, window: Number(w), date: base, dateISO });
    }
  }

  for (const b of bills) {
    // nextDue may be stale (set at creation, never refreshed). If it's in the
    // past or missing, recompute from dueDay so we always notify on the next
    // upcoming cycle rather than silently skipping.
    let base = parseDateFlexible(b.nextDue);
    if (!base || base < now) {
      const fresh = computeBillNextDue(b, now);
      base = parseDateFlexible(fresh);
    }
    if (!base) continue;
...
function isOurReminder(n) {
  ...
  return d?.app === "sublytics" && ["renewal", "bill", "trial"].includes(d?.kind);
}
...
export async function scheduleAll({
  ...
  quietStart = "22:00",
  quietEnd = "07:00",
  cap = 60, // iOS keeps at most 64 pending local notifications - stay under it
}) {
  // Permissions can throw (denied / restricted). Don't let that bubble up and
  // get silently swallowed by a fire-and-forget caller - report it instead.
  try {
    await ensureNotificationPerms();
  } catch (e) {
    return { scheduled: 0, failed: 0, errors: [], error: e?.message || "permissions_unavailable" };
  }
  await ensureNotificationCategory();

  await clearScheduledSublyticsReminders();

  if (!enabled) return { scheduled: 0, failed: 0, errors: [] };
  ...
  for (const x of toSchedule) {
    const { title, body } =
      x.kind === "trial"
        ? buildTrialCopy(x.name, x.window, x.date, x.amount, x.currency)
        : x.kind === "bill"
    ...
    try {
      ...
      // One bad item must never wipe out the whole batch.
      errors.push({ id: x.id, kind: x.kind, window: x.window, message: e?.message || String(e) });
    }
  }

  return { scheduled: count, failed: errors.length, errors, preview: upcoming.slice(0, 8) };
}
...
export async function handleNotificationAction(response) {
  ...
  // New potential subscription detected by a scan → open the review screen
  if (data.kind === "new_subscription") {
    if (actionId === "VIEW_DETAILS" || actionId === Notifications.DEFAULT_ACTION_IDENTIFIER) {
      return { ok: true, action: "view", screen: "/account/connect-email/review" };
    }
    return { ok: false, reason: "ignored" };
  }

  // Trial-ending alert (backend cron) → open the subscription detail.
  if (data.kind === "trial_ending") {
    if (actionId === "VIEW_DETAILS" || actionId === Notifications.DEFAULT_ACTION_IDENTIFIER) {
      return { ok: true, action: "view", screen: `/sub/${data.id}` };
    }
    return { ok: false, reason: "ignored" };
  }

  // Backend money alerts (price increase / charged-after-cancelling) → home,
  // where the subscription list and its changes surface.
  if (data.kind === "price_change" || data.kind === "zombie_charge") {
    if (actionId === "VIEW_DETAILS" || actionId === Notifications.DEFAULT_ACTION_IDENTIFIER) {
      return { ok: true, action: "view", screen: "/(tabs)" };
    }
    return { ok: false, reason: "ignored" };
  }
