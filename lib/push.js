...
/**
 * Syncs the user's "New subscription alerts" preference to all their
 * push_tokens rows, so the backend (which sends new-subscription pushes) honors
 * the toggle. Best-effort: never throws. Called when the toggle changes.
 */
export async function updateNewSubAlertPref(enabled) {
  try {
    const { data: userData } = await supabase.auth.getUser();
    const user = userData?.user;
